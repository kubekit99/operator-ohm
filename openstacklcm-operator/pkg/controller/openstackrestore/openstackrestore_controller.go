...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	// JEB: When a Workflow owned by OpenstackRestore is deleted, the Reconcile method is invoked.
...
func (r *ReconcileOpenstackRestore) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	myFinalizerName := "finalizer.openstackrestore"
	...
	if instance.ObjectMeta.DeletionTimestamp.IsZero() {
		...
		if !lcmutils.FinalizerContainsString(instance.ObjectMeta.Finalizers, myFinalizerName) {
			reqLogger.Info("Handling OpenstackRestore creation/update. Adding Finalizer")
			...
			reqLogger.Info("Handling OpenstackRestore creation/update")
		...
		if lcmutils.FinalizerContainsString(instance.ObjectMeta.Finalizers, myFinalizerName) {
			reqLogger.Info("Handling OpenstackRestore deletion step 1")
			...
			reqLogger.Info("Handling OpenstackRestore deletion step 2")
		}
	...
	// JEB: Check the OwnerReferenec by running "kubectl describe workflows/openstackrestore-wf"
	// JEB: The OpenstackRestore CR is owner of the workflow. This currently makes the Finalizer almost useless.
