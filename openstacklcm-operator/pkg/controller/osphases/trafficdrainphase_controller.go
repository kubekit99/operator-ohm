...
func newTrafficDrainPhaseReconciler(mgr manager.Manager) reconcile.Reconciler {
	r := &TrafficDrainPhaseReconciler{PhaseReconciler: PhaseReconciler{
...
func addTrafficDrainPhase(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	dependentPredicate := crtpredicate.Funcs{
		...
		CreateFunc: func(e event.CreateEvent) bool {
			...
			trafficdrainphaselog.Info("CreateEvent. Filtering", "resource", o.GetName(), "namespace", o.GetNamespace(),
		...
		DeleteFunc: func(e event.DeleteEvent) bool {
			o := e.Object.(*unstructured.Unstructured)
			trafficdrainphaselog.Info("DeleteEvent. Triggering", "resource", o.GetName(), "namespace", o.GetNamespace(),
				"apiVersion", o.GroupVersionKind().GroupVersion(), "kind", o.GroupVersionKind().Kind)
			return true
		},
		...
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			if reflect.DeepEqual(old.Object, new.Object) {
				trafficdrainphaselog.Info("UpdateEvent. Filtering", "resource", new.GetName(), "namespace", new.GetNamespace(),
					"apiVersion", new.GroupVersionKind().GroupVersion(), "kind", new.GroupVersionKind().Kind)
				return false
			} else {
				trafficdrainphaselog.Info("UpdateEvent. Triggering", "resource", new.GetName(), "namespace", new.GetNamespace(),
					"apiVersion", new.GroupVersionKind().GroupVersion(), "kind", new.GroupVersionKind().Kind)
				return true
			}
		},
...
// isReconcileDisabled
func (r TrafficDrainPhaseReconciler) isReconcileDisabled(instance *av1.TrafficDrainPhase) bool {
	...
	if instance.IsDisabled() {
		hrc := av1.LcmResourceCondition{
			...
			Status: av1.ConditionStatusFalse,
			Reason: "TrafficDrainPhase is disabled",
		}
		r.recorder.Event(instance, corev1.EventTypeWarning, hrc.Type.String(), hrc.Reason.String())
		instance.Status.SetCondition(hrc, instance.Spec.TargetState)
		_ = r.updateResourceStatus(instance)
		...
		hrc := av1.LcmResourceCondition{
			Type:   av1.ConditionEnabled,
			Status: av1.ConditionStatusTrue,
			Reason: "TrafficDrainPhase is enabled",
		}
		r.recorder.Event(instance, corev1.EventTypeNormal, hrc.Type.String(), hrc.Reason.String())
		instance.Status.SetCondition(hrc, instance.Spec.TargetState)
		return false
